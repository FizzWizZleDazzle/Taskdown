...
impl AuthService {
        ...
        match auth_config.r#type.as_str() {
            "api-key" | "bearer" => {
                if let Some(token) = &auth_config.token {
                    ...
                    Err(anyhow::anyhow!(
                        "Token is required for API key authentication"
                    ))
                }
            }
            "basic" => {
            ...
            "custom" => {
                if let Some(headers) = &auth_config.custom_headers {
                    ...
                    Err(anyhow::anyhow!(
                        "Custom headers are required for custom authentication"
                    ))
                }
            }
            _ => Err(anyhow::anyhow!(
    ...
    async fn verify_custom_auth(
        ...
        if let Some(auth_token) = headers.get("X-Auth-Token") {
            ...
            Err(anyhow::anyhow!(
                "Missing X-Auth-Token header for custom authentication"
            ))
        }
    }
}
